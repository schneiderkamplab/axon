...
def _synapse_module() -> Any:
    return importlib.import_module("synapse")


def _axon_module() -> Any:
    return importlib.import_module("synapse.axon")
...
def _axon_codegen_dump_to_text(
    ...
    *,
    ...
    if model_dir is not None:
        axon_test_module = importlib.import_module("synapse.axon_test")
    ...
    if backend == "codegen2-torch":
        emit_module = importlib.import_module("synapse.axon.codegen2_torch")
    ...
    emit_module = importlib.import_module("synapse.axon.codegen2_mlx")
    emit_fn = getattr(emit_module, "emit_model_code_from_graph_ir")
    return emit_fn(graph, class_name=class_name, model_config=model_config)
...
def axon_benchmark(
    ...
    axon_backend: str = typer.Option(
        ...
        help=(
            ...
            "codegen2-mlx, codegen2-jax, codegen2-triton, codegen2-vllm, runtime2-torch, or pipeline2-torch)."
        ),
    ),
