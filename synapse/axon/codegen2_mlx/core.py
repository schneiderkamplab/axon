...
class _DirectMlxEmitter(_DirectTorchEmitter):
    ...
    def _emit_build_ff(self, lines: list[str]) -> None:
        ...
        add(lines, 12, "_hrm_keys = [k for k in _ft if '.gqkv_proj.weight' in k]")
        add(lines, 12, "if _hrm_keys and 'model.z_L_init' in _ft:")
        add(lines, 16, "cfg = self.config")
        add(lines, 16, "_hdm = int(cfg.get('hidden_size', 1536))")
        add(lines, 16, "_nh = int(cfg.get('num_attention_heads', 12))")
        add(lines, 16, "_hd = int(cfg.get('head_dim', _hdm // _nh))")
        add(lines, 16, "_qd = _nh * _hd")
        add(lines, 16, "_ffn = int(cfg.get('intermediate_size', 4096))")
        add(lines, 16, "_hc = int(cfg.get('H_cycles', 2))")
        add(lines, 16, "_lc = int(cfg.get('L_cycles', 3))")
        add(lines, 16, "_nl = int(cfg.get('num_hidden_layers', 16))")
        add(lines, 16, "_eps = float(cfg.get('rms_norm_eps', 1e-6))")
        add(lines, 16, "_theta = float(cfg.get('rope_theta', 10000.0))")
        add(lines, 16, "_es = float(cfg.get('embedding_scale', 1.0))")
        add(lines, 16, "_ts = _hc * (_lc + 1) * _nl")
        add(lines, 16, "p = self._param")
        add(lines, 16, "embed_w = p('model.embed_tokens.weight')")
        add(lines, 16, "lm_head_w = p('lm_head.weight')")
        add(lines, 16, "z_L_init = p('model.z_L_init')")
        add(lines, 16, "_half = _hd // 2")
        add(lines, 16, "_freqs = 1.0 / mx.exp(-math.log(_theta) * (2.0 * mx.arange(_half, dtype=mx.float32) / _hd))")
        add(lines, 16, "")
        add(lines, 16, "def _rms(x):")
        add(lines, 20, "return mx.fast.rms_norm(x, None, _eps)")
        add(lines, 16, "")
        add(lines, 16, "def _block(x, mask, past_i, wp, B, S, off):")
        add(lines, 20, "xn = _rms(x)")
        add(lines, 20, "gqkv = xn @ p(wp + '.attn.gqkv_proj.weight').swapaxes(-1, -2)")
        add(lines, 20, "gl, ql, kl, vl = mx.split(gqkv, 4, axis=-1)")
        add(lines, 20, "q = ql.reshape(B, S, _nh, _hd).transpose(0, 2, 1, 3)")
        add(lines, 20, "k = kl.reshape(B, S, _nh, _hd).transpose(0, 2, 1, 3)")
        add(lines, 20, "v = vl.reshape(B, S, _nh, _hd).transpose(0, 2, 1, 3)")
        add(lines, 20, "g = gl.reshape(B, S, _nh, _hd).transpose(0, 2, 1, 3)")
        add(lines, 20, "q = mx.fast.rope(q, _hd, traditional=False, base=None, scale=1.0, offset=int(off), freqs=_freqs)")
        add(lines, 20, "k = mx.fast.rope(k, _hd, traditional=False, base=None, scale=1.0, offset=int(off), freqs=_freqs)")
        add(lines, 20, "if isinstance(past_i, _KVCache):")
        add(lines, 24, "k_all, v_all = past_i.update_and_fetch(k, v)")
        add(lines, 24, "new_layer = past_i")
        add(lines, 20, "elif past_i is None:")
        add(lines, 24, "kvc = _KVCache()")
        add(lines, 24, "k_all, v_all = kvc.update_and_fetch(k, v)")
        add(lines, 24, "new_layer = kvc")
        add(lines, 20, "else:")
        add(lines, 24, "kp, vp = past_i")
        add(lines, 24, "k_all = mx.concatenate([kp, k], axis=2)")
        add(lines, 24, "v_all = mx.concatenate([vp, v], axis=2)")
        add(lines, 24, "new_layer = (k_all, v_all)")
        add(lines, 20, "a = mx.fast.scaled_dot_product_attention(q, k_all, v_all, mask=mask, scale=1.0/(_hd**0.5))")
        add(lines, 20, "a = mx.sigmoid(g) * a")
        add(lines, 20, "a = a.transpose(0, 2, 1, 3).reshape(B, S, _nh * _hd)")
        add(lines, 20, "a = a @ p(wp + '.attn.o_proj.weight').swapaxes(-1, -2)")
        add(lines, 20, "x = x + a")
        add(lines, 20, "xn = _rms(x)")
        add(lines, 20, "gu = xn @ p(wp + '.mlp.gate_up_proj.weight').swapaxes(-1, -2)")
        add(lines, 20, "gm, um = mx.split(gu, 2, axis=-1)")
        add(lines, 20, "m = (mx.sigmoid(gm) * gm) * um")
        add(lines, 20, "m = m @ p(wp + '.mlp.down_proj.weight').swapaxes(-1, -2)")
        add(lines, 20, "x = x + m")
        add(lines, 20, "return x, new_layer")
        add(lines, 16, "")
        add(lines, 16, "def _stack(x, mask, rkv, wkv, co, mod, B, S, off):")
        add(lines, 20, "for i in range(_nl):")
        add(lines, 24, "slot = co + i")
        add(lines, 24, "pi = rkv[slot] if rkv is not None and slot < len(rkv) else None")
        add(lines, 24, "x, nl = _block(x, mask, pi, f'model.{mod}.layers.{i}', B, S, off)")
        add(lines, 24, "wkv[slot] = nl")
        add(lines, 20, "return _rms(x)")
        add(lines, 16, "")
        add(lines, 16, "def _ff(input_ids, past_kv=None, use_cache=False):")
        add(lines, 20, "B, S = input_ids.shape")
        add(lines, 20, "off = 0")
        add(lines, 20, "if past_kv is not None:")
        add(lines, 24, "for _pkv in past_kv:")
        add(lines, 28, "if _pkv is not None:")
        add(lines, 32, "off = _pkv.offset if isinstance(_pkv, _KVCache) else _pkv[0].shape[2]")
        add(lines, 32, "break")
        add(lines, 20, "x = mx.take(embed_w, input_ids, axis=0).reshape(B, S, _hdm)")
        add(lines, 20, "if abs(_es - 1.0) > 1e-6: x = x * _es")
        add(lines, 20, "kl = off + S")
        add(lines, 20, "if S == 1: mask = None")
        add(lines, 20, "else: mask = (mx.arange(kl, dtype=mx.int32).reshape(1, kl) <= (mx.arange(S, dtype=mx.int32).reshape(S, 1) + off)).reshape(1, 1, S, kl)")
        add(lines, 20, "z_l = mx.broadcast_to(z_L_init.reshape(1, 1, _hdm), (B, S, _hdm)).astype(x.dtype)")
        add(lines, 20, "z_h = x")
        add(lines, 20, "rkv = list(past_kv) if past_kv is not None else [None] * _ts")
        add(lines, 20, "wkv = [None] * _ts")
        add(lines, 20, "so = 0")
        add(lines, 20, "for _ in range(_hc):")
        add(lines, 24, "for _ in range(_lc):")
        add(lines, 28, "z_l = _stack(z_l + z_h, mask, rkv, wkv, so, 'L_module', B, S, off)")
        add(lines, 28, "so += _nl")
        add(lines, 24, "z_h = _stack(z_h + z_l, mask, rkv, wkv, so, 'H_module', B, S, off)")
        add(lines, 24, "so += _nl")
        add(lines, 20, "logits = z_h @ lm_head_w.swapaxes(-1, -2)")
        add(lines, 20, "return logits, (wkv if use_cache else None)")
        add(lines, 16, "")
        add(lines, 16, "return _ff")
    ...
    def _emit_common(self, lines: list[str]) -> None:
        ...
        add(lines, 20, "raw[str(key)] = f.get_tensor(key)")
        add(lines, 8, "# int8+scale pairs must reach load_state_dict uncast so dequantization")
        add(lines, 8, "# stays bit-identical to the published dequant checkpoints")
        add(lines, 8, "quant_keys = set()")
        add(lines, 8, "for key in raw:")
        add(lines, 12, "if key.endswith('.int8') and f\"{key[: -len('.int8')]}.scale\" in raw:")
        add(lines, 16, "quant_keys.add(key)")
        add(lines, 16, "quant_keys.add(f\"{key[: -len('.int8')]}.scale\")")
        add(lines, 8, "for key, t in raw.items():")
        add(lines, 12, "if t.dtype == torch.bfloat16:")
        add(lines, 16, "arr = mx.array(t.float().numpy()).astype(mx.bfloat16)")
        add(lines, 12, "else:")
        add(lines, 16, "arr = mx.array(t.numpy())")
        add(lines, 12, "if target is not None and key not in quant_keys:")
        add(lines, 16, "arr = arr.astype(target)")
        add(lines, 12, "state_dict[key] = arr")
        add(lines, 8, "return cls(state_dict, config=_MODEL_CONFIG if model_config is None else model_config, target_dtype=dtype)")
        add(lines, 4, "")
        ...
        add(lines, 12, "deq = q.astype(mx.float32) * mx.expand_dims(scale.astype(mx.float32), 1)")
        add(lines, 12, "state_dict[base] = deq.astype(mx.bfloat16)")
        add(lines, 12, "del state_dict[key]")
        ...
        add(lines, 4, "")
        add(lines, 4, "def _mlx_rope_apply(self, x, sin, cos, traditional=False):")
        ...
        add(lines, 12, "else:")
        add(lines, 16, "if traditional:")
        add(lines, 20, "x_rs = x.reshape(*x.shape[:-1], half, 2)")
        add(lines, 20, "x_even = x_rs[..., 0]")
        add(lines, 20, "x_odd = x_rs[..., 1]")
        add(lines, 20, "rotated = mx.concatenate([-x_odd[..., None], x_even[..., None]], axis=-1).reshape(*x.shape)")
        add(lines, 16, "else:")
        add(lines, 20, "x1 = x[..., :half]")
        add(lines, 20, "x2 = x[..., half:]")
        add(lines, 20, "rotated = mx.concatenate([-x2, x1], axis=-1)")
        ...
        add(lines, 8, "if S > 1:")
        add(lines, 12, "return mx.fast.rope(x, dim, traditional=traditional, base=None, scale=1.0, offset=0, freqs=freqs)")
        add(lines, 8, "else:")
        ...
        add(lines, 12, "return mx.fast.rope(x, dim, traditional=traditional, base=None, scale=1.0, offset=offset, freqs=freqs)")
        add(lines, 4, "")
        add(lines, 4, "")
        ...
        add(lines, 4, "")
        add(lines, 4, "@staticmethod")
        add(lines, 4, "def _split_to_sections(x, sizes, axis=-1):")
        add(lines, 8, "import itertools")
        add(lines, 8, "cumulative = list(itertools.accumulate(int(s) for s in sizes))")
        add(lines, 8, "return mx.split(x, indices_or_sections=cumulative[:-1], axis=int(axis))")
        add(lines, 4, "")
        add(lines, 4, "@staticmethod")
        ...
        add(lines, 8, "# torch.topk semantics: (values, indices), descending when largest.")
        add(lines, 8, "# mx.topk alone is values-only and ascending, so build from argpartition.")
        add(lines, 8, "k = int(k)")
        add(lines, 8, "axis = int(dim)")
        add(lines, 8, "keys = -x if largest else x")
        add(lines, 8, "idx = mx.take(mx.argpartition(keys, kth=k - 1, axis=axis), mx.arange(k), axis=axis)")
        add(lines, 8, "vals = mx.take_along_axis(x, idx, axis=axis)")
        add(lines, 8, "if sorted_:")
        add(lines, 12, "order = mx.argsort(-vals if largest else vals, axis=axis)")
        add(lines, 12, "idx = mx.take_along_axis(idx, order, axis=axis)")
        add(lines, 12, "vals = mx.take_along_axis(vals, order, axis=axis)")
        add(lines, 8, "return (vals, idx)")
        add(lines, 4, "")
        add(lines, 4, "@staticmethod")
        ...
        add(lines, 4, "")
        add(lines, 4, "@staticmethod")
        add(lines, 4, "def _gegelu_split(x, limit=None):")
        add(lines, 8, "gate, up = mx.split(x, 2, axis=-1)")
        add(lines, 8, "if limit is not None:")
        add(lines, 12, "limit_val = float(limit)")
        add(lines, 12, "gate = mx.minimum(gate, limit_val)")
        add(lines, 12, "up = mx.clip(up, -limit_val, limit_val)")
        add(lines, 8, "return mx.sigmoid(1.702 * gate) * gate * (up + 1.0)")
        add(lines, 4, "")
        add(lines, 4, "@staticmethod")
    ...
    def _emit_load_state_dict(self, lines: list[str]) -> None:
        ...
        add(lines, 12, "self._quantized = True")
        add(lines, 12, "quantized_types = tuple(t for t in (getattr(nn, 'QuantizedLinear', None), getattr(nn, 'QuantizedEmbedding', None)) if t is not None)")
        add(lines, 12, "if not any(isinstance(m, quantized_types) for m in self.modules()):")
        add(lines, 16, "import warnings as _w")
        add(lines, 16, "_w.warn('quantize=True had no effect: weights are held as flat tensors, not nn.Linear/nn.Embedding submodules; use an int8 checkpoint (*.int8 + *.scale), which is dequantized on load', RuntimeWarning, stacklevel=2)")
    ...
    def _emit_forward(self, lines: list[str]) -> None:
        ...
        add(lines, 8, "_num_layers_sym = self._symbols.get('NUM_LAYERS')")
        add(lines, 8, "kv = [None] * int(_num_layers_sym) if _num_layers_sym is not None else None")
    ...
    def _primitive_expr(self, primitive: str, node: Any, *, local: set[str], symbols_dict: str) -> str:
        ...
        if primitive == "_mlx_gegelu_activation":
            if len(args) < 2:
                raise ValueError("__mlx_gegelu_activation expects packed input and limit")
            return f"self._gegelu_split({args[0]}, {args[1]})"
        ...
        if primitive == "_mlx_rope":
            if len(args) >= 5:
                return f"(self._mlx_rope_apply({args[0]}, {args[3]}, {args[4]}, traditional=bool({args[2]})), self._mlx_rope_apply({args[1]}, {args[3]}, {args[4]}, traditional=bool({args[2]})))"
            if len(args) >= 4:
                return f"self._mlx_rope_apply({args[0]}, {args[2]}, {args[3]}, traditional=bool({args[1]}))"
        ...
        simple = {
            ...
            "split": lambda: f"self._split_to_sections({args[0]}, {args[2] if len(args) > 2 else attrs.get('sizes', '[]')}, axis=int({args[1] if len(args) > 1 else attrs.get('dim', '-1')}))",
            ...
            "gather": lambda: f"mx.take_along_axis({args[0]}, {args[1]}.astype(mx.int64), axis=int({args[2] if len(args) > 2 else '-1'}))",
