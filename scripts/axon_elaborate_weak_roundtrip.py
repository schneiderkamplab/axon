...
from synapse.axon.validate import validate_normalized_axon_file
...
def _parse_normalize_elaborate_render(source: str, *, validate: bool) -> str:
    elaborated = elaborate_closed_axon_file(
...
def elaborate_weak_roundtrip_path(
    path: Path,
    output_dir: Path,
    *,
    ...
    second = _parse_normalize_elaborate_render(first, validate=validate_normalized)
    write_generation(output_dir, "render2", path, second)

    third = _parse_normalize_elaborate_render(second, validate=validate_normalized)
    write_generation(output_dir, "render3", path, third)

    return first == second == third


def run_elaborate_weak_roundtrip(
    paths: list[Path],
    *,
    output_dir: Path = Path("tmp/axon-stage-roundtrip-elaborate-weak"),
    ...
    return run_path_roundtrips(
        ...
        label="elaborate-weak",
        roundtrip_path=lambda path, out: elaborate_weak_roundtrip_path(
            path,
            out,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Weak elaborate roundtrip: render, reparse, renormalize, "
            "re-elaborate without reresolve."
        )
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("tmp/axon-stage-roundtrip-elaborate-weak"),
    )
    ...
    result = run_elaborate_weak_roundtrip(
        args.paths,
