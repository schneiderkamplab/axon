...
def typecheck2_weak_roundtrip_path(
    path: Path,
    output_dir: Path,
    *,
...
def run_typecheck2_weak_roundtrip(
    paths: list[Path],
    *,
    main_module: str | None = None,
    output_dir: Path = Path("tmp/axon-stage-roundtrip-typecheck2-weak"),
    ...
    return run_path_roundtrips(
        ...
        label="typecheck2-weak",
        roundtrip_path=lambda path, out: typecheck2_weak_roundtrip_path(
            path,
            out,
            main_module=main_module,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Weak typecheck2 roundtrip: render, reparse, renormalize, retypecheck without reresolve/reflatten."
    )
    parser.add_argument("paths", nargs="*", type=Path, help="Axon files or directories.")
    parser.add_argument("--main-module")
    parser.add_argument("--output-dir", type=Path, default=Path("tmp/axon-stage-roundtrip-typecheck2-weak"))
    ...
    result = run_typecheck2_weak_roundtrip(
        args.paths,
