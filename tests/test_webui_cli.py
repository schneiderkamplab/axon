...
def test_webui_opens_browser_and_serves(monkeypatch) -> None:
    ...
    monkeypatch.setattr(webui_module, "_serve_webui", lambda *, host, port: served.append((host, port)))

    webui_module.webui(host="127.0.0.1", port=8766, log_level="info", open_browser=True)
