...
def test_structured_path_matcher_rejects_invalid_output_patterns() -> None:
    ...
    with pytest.raises(_MatchError, match="regex token not allowed"):
        matcher.rewrite(["~x::.*"], match)

    with pytest.raises(_MatchError, match="unknown interpolation variable"):
...
def test_structured_path_matcher_rejects_invalid_capture_and_regex_forms() -> None:
    ...
    with pytest.raises(_MatchError, match="invalid capture name"):
        matcher.match(["$1bad"], "x")

    with pytest.raises(_MatchError, match="invalid regex token"):
        matcher.match(["~::(x)"], "x")

    with pytest.raises(_MatchError, match="missing regex body"):
        matcher.match(["~x::"], "x")

    with pytest.raises(_MatchError, match="invalid structured regex"):
        matcher.match(["~x::("], "x")
...
def test_structured_path_matcher_rejects_invalid_variadic_output_bindings() -> None:
    ...
    with pytest.raises(_MatchError, match="unknown variadic variable"):
        matcher.rewrite(["*tail"], StructuredMatch(bindings={"x": "1"}))

    with pytest.raises(_MatchError, match="is not variadic"):
        matcher.rewrite(["*tail"], StructuredMatch(bindings={"tail": "x"}))

    with pytest.raises(_MatchError, match="contains non-string segments"):
