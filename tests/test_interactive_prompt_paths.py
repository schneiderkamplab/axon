...
def test_prompt_interactive_transform_multiline_block_success(
    ...
    monkeypatch.setattr(
        interactive, "_parse_transform_block", lambda block: [{"copy": {"from": "a", "to": "b"}}]
    )
    history: list[str] = []
    monkeypatch.setattr(interactive, "_add_history_entry", history.append)
    monkeypatch.setattr("builtins.input", lambda _prompt: next(lines))
    result = interactive._prompt_interactive_transform(state_dict_provider=None)
