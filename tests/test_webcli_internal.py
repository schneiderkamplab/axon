...
def test_webcli_callback_handles_browser_error_and_wildcard_host(
    ...
    monkeypatch.setattr(
        webcli_cli, "_serve_webcli", lambda *, host, port: served.append((host, port))
    )
...
def test_webcli_handler_do_post_error_branches(
    ...
    monkeypatch.setattr(
        webcli_handler, "_run_web_plan", lambda **_kwargs: (_ for _ in ()).throw(exc)
    )
    handler_cls.do_POST(h)
    assert h._json
    payload = h._json[-1][1]
...
def test_webcli_runner_branches(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        webcli_runner, "compile_plan", lambda raw: _FakePlan(output=raw.get("output"))
    )
    monkeypatch.setattr(
        webcli_runner, "create_state_dict_provider", lambda **_kwargs: fake_provider
    )
    monkeypatch.setattr(
        webcli_runner,
        "normalize_raw_plan",
    ...
    monkeypatch.setattr(
        webcli_runner, "create_state_dict_provider", lambda **_kwargs: fake_provider2
    )
    ...
    monkeypatch.setattr(
        webcli_runner, "create_state_dict_provider", lambda **_kwargs: fake_provider3
    )
    monkeypatch.setattr(webcli_runner, "get_runtime_flags", lambda: SimpleNamespace(dry_run=True))
    ...
    monkeypatch.setattr(
        webcli_runner, "create_state_dict_provider", lambda **_kwargs: fake_provider4
    )
