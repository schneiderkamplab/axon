...
from brainsurgery.core.specs import select_tensor
...
def test_validate_expr_kind_and_helpers() -> None:
    ...
    assert (
        format_tensor_ref(TensorRef(model="base", expr="weight", slice_spec="[:2]"))
        == "base::weight::[:2]"
    )
    assert (
        format_tensor_ref(TensorRef(model="base", expr=["layer", "$i"], slice_spec=None))
        == "base::['layer', '$i']"
    )
