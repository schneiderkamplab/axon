...
def test_save_compile_additional_validation_paths() -> None:
    ...
    with pytest.raises(SaveTransformError, match="must not be sliced"):
        SaveTransform().compile({"path": "/tmp/x.safetensors", "target": "model::x::[:1]"}, default_model="model")
    original_parse_model_expr = _module.parse_model_expr
    try:
        _module.parse_model_expr = lambda raw, default_model=None: TensorRef(model="model", expr=["x"], slice_spec=None)
        with pytest.raises(SaveTransformError, match="single tensor name"):
            SaveTransform().compile({"path": "/tmp/x.safetensors", "target": "model::x"}, default_model="model")
    finally:
...
def test_save_apply_and_helper_error_paths(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    provider = _Provider()
    state_spec = SaveSpec(path=Path("/tmp/out.safetensors"), alias="model", tensor_name=None, format=None, shard_size=None)
    monkeypatch.setattr(_module, "persist_state_dict", lambda *args, **kwargs: (_ for _ in ()).throw(RuntimeError("write failed")))
    with pytest.raises(SaveTransformError, match="write failed"):
        SaveTransform().apply(state_spec, provider)  # type: ignore[arg-type]

    missing_spec = SaveSpec(path=Path("/tmp/out.safetensors"), alias="model", tensor_name="missing", format=None, shard_size=None)
    ...
    tensor_spec = SaveSpec(path=Path("/tmp/out.npy"), alias="model", tensor_name="x", format="numpy", shard_size=None)
    monkeypatch.setattr(_module, "save_tensor_to_path", lambda *args, **kwargs: (_ for _ in ()).throw(RuntimeError("tensor failed")))
    ...
    monkeypatch.setattr(_module, "parse_shard_size", lambda raw: (_ for _ in ()).throw(RuntimeError("output.shard invalid")))
