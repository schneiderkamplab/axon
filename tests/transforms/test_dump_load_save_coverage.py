...
def test_dump_apply_all_formats_and_helpers(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    sliced = DumpTransform().build_spec(ref, {"format": "tree", "verbosity": "full"}, dump_all_models=False)
...
def test_load_transform_completion_and_success_paths(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    tensor_spec = LoadSpec(path=Path("/tmp/t.npy"), alias="model", tensor_name="new_x", format="numpy")
    monkeypatch.setattr("brainsurgery.transforms.load.load_tensor_from_path", lambda path, format: torch.zeros(1))
