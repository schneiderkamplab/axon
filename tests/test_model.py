...
def test_load_torch_distributed_checkpoint_rejects_zero_tensors(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_is_full_tensor_storage_metadata_handles_invalid_chunks() -> None:
    ...
    assert checkpoint_io._is_full_tensor_storage_metadata(entry) is False


def test_load_dcp_direct_rejects_invalid_metadata_non_dict(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    class _Reader:
        def __init__(self, _path: str):
            pass
...
def test_load_dcp_direct_rejects_missing_dtype(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    from torch.distributed.checkpoint.metadata import (
        ChunkStorageMetadata,
        TensorProperties,
        TensorStorageMetadata,
    ...
    monkeypatch.setattr("torch.distributed.checkpoint.FileSystemReader", _Reader)
    monkeypatch.setattr(checkpoint_io.torch, "dtype", tuple)  # force isinstance(dtype, torch.dtype) to fail
    with pytest.raises(RuntimeError, match="missing dtype"):
        checkpoint_io._load_state_dict_from_torch_distributed_checkpoint_direct(tmp_path / "dcp")


def test_load_dcp_via_conversion_import_error(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    original_import = builtins.__import__
    ...
    with pytest.raises(RuntimeError, match="requires torch.distributed.checkpoint.format_utils"):
        checkpoint_io._load_state_dict_from_torch_distributed_checkpoint_via_conversion(tmp_path / "dcp")
